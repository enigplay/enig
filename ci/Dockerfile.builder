...
# enig_hash
RUN git clone https://github.com/raptor3um/enig_hash
RUN cd enig_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} enig
RUN useradd -u ${USER_ID} -g enig -s /bin/bash -m -d /enig enig
...
RUN mkdir /enig-src && \
  ...
  chown $USER_ID:$GROUP_ID /enig-src && \
