...
{
    return QCoreApplication::translate("enig-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in enig.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in enig.qrc)
...
/** Class encapsulating Enig Core startup and shutdown.
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Enig Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Enig Core - \""), false, OptionsCategory::GUI);
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(enig);
    Q_INIT_RESOURCE(enig_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse enig.conf
    ...
    // enig: links repeatedly have their payment requests routed to this process:
